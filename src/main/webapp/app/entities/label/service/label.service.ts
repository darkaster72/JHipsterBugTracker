...
export class LabelService {
  ...
  update(label: ILabel): Observable<EntityResponseType> {
    return this.http.put<ILabel>(`${this.resourceUrl}/${getLabelIdentifier(label) as string}`, label, { observe: 'response' });
  }

  partialUpdate(label: ILabel): Observable<EntityResponseType> {
    return this.http.patch<ILabel>(`${this.resourceUrl}/${getLabelIdentifier(label) as string}`, label, { observe: 'response' });
  }
